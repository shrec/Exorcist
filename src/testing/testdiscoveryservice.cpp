...
void TestDiscoveryService::runNextTest()
{
    ...
    // On Windows, Qt test binaries linked with Qt6::Widgets use the GUI
    // subsystem — stdout is not a real pipe.  Pass -o <file>,txt to the binary
    // so it writes output to a file, then read that file back.
    ...
    // Append -o only when the test hasn't already specified output redirection
    ...
    // Prefer the output file written by -o; fall back to captured stdout/stderr
    QFile f(outFile);
