#include "trajectoryreplaywidget.h"
...
TrajectoryReplayWidget::TrajectoryReplayWidget(QWidget *parent)
    : QWidget(parent)
{
...
void TrajectoryReplayWidget::loadTrajectory(const QJsonArray &steps)
{
    m_steps = steps;
    m_currentStep = 0;
...
void TrajectoryReplayWidget::loadFile()
{
...
void TrajectoryReplayWidget::nextStep()
{
...
void TrajectoryReplayWidget::prevStep()
{
    if (m_currentStep > 0) {
        m_currentStep--;
        m_slider->setValue(m_currentStep);
...
void TrajectoryReplayWidget::togglePlay()
{
    if (m_timer->isActive()) {
        m_timer->stop();
        m_playBtn->setText(QStringLiteral("\u25B6"));
...
void TrajectoryReplayWidget::showStep()
{
    if (m_steps.isEmpty()) {
        m_content->clear();
