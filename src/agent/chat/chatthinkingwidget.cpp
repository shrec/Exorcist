...
ChainLineWidget::ChainLineWidget(const QString &icon, QWidget *parent)
    : QWidget(parent), m_icon(icon)
{
    setFixedWidth(22);
}

void ChainLineWidget::setShowLine(bool top, bool bottom)
{
    m_topLine    = top;
    m_bottomLine = bottom;
    update();
...
void ChainLineWidget::paintEvent(QPaintEvent *)
{
    QPainter p(this);
...
ChatThinkingWidget::ChatThinkingWidget(QWidget *parent)
    : QWidget(parent)
{
...
void ChatThinkingWidget::setThinkingText(const QString &text, bool streaming)
{
    m_rawText = text;
...
void ChatThinkingWidget::appendDelta(const QString &delta)
{
    m_rawText += delta;
    m_streaming = true;
...
void ChatThinkingWidget::finalize()
{
    m_streaming = false;
...
void ChatThinkingWidget::setCollapsed(bool collapsed)
{
    m_headerBtn->setChecked(!collapsed);
}

ChatThinkingWidget::ToolItem *ChatThinkingWidget::addToolItem(const ChatContentPart &part)
{
...
void ChatThinkingWidget::updateToolState(const QString &callId, const ChatContentPart &part)
{
...
void ChatThinkingWidget::updateBoxStyle(bool streaming)
{
...
void ChatThinkingWidget::updateHeaderStyle(bool expanded)
{
    Q_UNUSED(expanded)
...
QString ChatThinkingWidget::toolIcon(const QString &name)
{
