...
private:
    ...
    // Shows tool-specific summary instead of raw JSON dump.
    ...
    // ── JSON rendering with syntax coloring ──────────────────────────────
    ...
    {
        // Preserve line breaks, escape HTML, truncate
    ...
    static QString iconForTool(const QString &toolName)
    {
        if (toolName.contains(QLatin1String("grep"))
            ...
            return QStringLiteral("\U0001F50D");  // magnifying glass
        if (toolName.contains(QLatin1String("read")))
            return QStringLiteral("\U0001F4C4");  // page
        if (toolName.contains(QLatin1String("list")))
            return QStringLiteral("\U0001F4C2");  // open folder
