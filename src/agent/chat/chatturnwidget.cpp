...
ChatTurnWidget::ChatTurnWidget(const ChatTurnModel &turn, QWidget *parent)
    : QWidget(parent)
    , m_turnId(turn.id)
{
...
void ChatTurnWidget::appendMarkdownDelta(const QString &delta)
{
    if (m_lastMarkdown) {
...
void ChatTurnWidget::appendThinkingDelta(const QString &delta)
{
    if (m_lastThinking) {
...
void ChatTurnWidget::addContentPart(const ChatContentPart &part)
{
    addPartWidget(part);
}

void ChatTurnWidget::updateToolState(const QString &callId, const ChatContentPart &part)
{
...
void ChatTurnWidget::finishTurn(ChatTurnModel::State state)
{
    if (m_lastMarkdown)
...
void ChatTurnWidget::setTokenUsage(int promptTokens, int completionTokens, int totalTokens)
{
    if (totalTokens <= 0) {
        m_usageLabel->hide();
        return;
...
void ChatTurnWidget::showError(const QString &errorText, AgentError::Code code)
{
...
void ChatTurnWidget::addPartWidget(const ChatContentPart &part)
{
    switch (part.type) {
...
QString ChatTurnWidget::feedbackBtnStyle()
{
...
bool ChatTurnWidget::eventFilter(QObject *obj, QEvent *event)
{
...
void ChatTurnWidget::enterEvent(QEnterEvent *event)
{
...
void ChatTurnWidget::leaveEvent(QEvent *event)
{
