...
MemoryTool::MemoryTool(const QString &baseDir)
    : m_baseDir(baseDir)
{
...
ToolExecResult MemoryTool::invoke(const QJsonObject &args)
{
...
ToolExecResult MemoryTool::ok(const QString &text)
{
    ToolExecResult r;
    r.ok = true;
    r.textContent = text;
...
ToolExecResult MemoryTool::fail(const QString &msg)
{
    return {false, {}, {}, msg};
}

QString MemoryTool::resolvePath(const QString &memPath) const
{
    QString rel = memPath;
...
ToolExecResult MemoryTool::doView(const QString &path) const
{
...
ToolExecResult MemoryTool::doCreate(const QString &path, const QJsonObject &args) const
{
    const QString resolved = resolvePath(path);
    if (resolved.isEmpty())
        return fail(QStringLiteral("Invalid path"));
...
ToolExecResult MemoryTool::doStrReplace(const QString &path, const QJsonObject &args) const
{
    const QString resolved = resolvePath(path);
    if (resolved.isEmpty())
        return fail(QStringLiteral("Invalid path"));
...
ToolExecResult MemoryTool::doInsert(const QString &path, const QJsonObject &args) const
{
    const QString resolved = resolvePath(path);
    if (resolved.isEmpty())
        return fail(QStringLiteral("Invalid path"));
...
ToolExecResult MemoryTool::doDelete(const QString &path) const
{
    const QString resolved = resolvePath(path);
    if (resolved.isEmpty())
        return fail(QStringLiteral("Invalid path"));
...
ToolExecResult MemoryTool::doRename(const QString &path, const QJsonObject &args) const
{
