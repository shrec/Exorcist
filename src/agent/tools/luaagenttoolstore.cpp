...
ToolSpec SaveLuaToolTool::spec() const
{
    ToolSpec s;
...
ToolExecResult SaveLuaToolTool::invoke(const QJsonObject &args)
{
...
ToolSpec ListLuaToolsTool::spec() const
{
    ToolSpec s;
...
ToolExecResult ListLuaToolsTool::invoke(const QJsonObject &)
{
    const QString dir = AgentToolStoreDetail::agentToolsDir();
...
RunLuaToolTool::RunLuaToolTool(LuaExecutor executor)
    : m_executor(std::move(executor)) {}

ToolSpec RunLuaToolTool::spec() const
{
    ToolSpec s;
...
ToolExecResult RunLuaToolTool::invoke(const QJsonObject &args)
{
    const QString rawName = args[QLatin1String("name")].toString();
    if (rawName.isEmpty())
        return {false, {}, {},
