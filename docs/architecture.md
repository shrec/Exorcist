...
| **Code Graph** | `codegraph/` | SQLite-based code intelligence engine — indexes files, classes, functions (with exact line ranges via brace-depth tracking), edges, Qt connections, services, test cases, CMake targets. 18-table schema + FTS5 search. Ported from `tools/build_codegraph.py`. | `CodeGraphDb`, `CodeGraphIndexer`, `CodeGraphQuery` |
