...
| **Build System** | `src/build/` + `plugins/build/` | Core types (`Kit`, `IKitManager`) in `src/build/`; **Core Plugin** provides CMake integration, toolchain detection, kit management, build toolbar, debug launch controller, output panel, run panel. Kit bundles compiler + debugger + cmake + generator into a single selectable profile. Auto-detected from local toolchains, user-defined kits persisted to QSettings. | `Kit`, `IKitManager` (core); `BuildPlugin`, `KitManager`, `CMakeIntegration`, `ToolchainManager`, `BuildToolbar`, `DebugLaunchController`, `BuildSystemService` (plugin) |
